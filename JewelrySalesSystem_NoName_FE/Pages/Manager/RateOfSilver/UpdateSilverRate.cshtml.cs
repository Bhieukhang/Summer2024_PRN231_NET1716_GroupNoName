...
{
    public class UpdateSilverRateModel : PageModel
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly IConfiguration _configuration;

        public UpdateSilverRateModel(IHttpClientFactory httpClientFactory, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> OnPostAsync([FromBody] double NewRate)
        {
            ...
            try
            {
                ...
                var response = await client.PostAsync(ApiPath.SilverRateList, content);
                ...
                {
                    return new JsonResult(new { success = true, message = "Silver rate updated successfully." });
                }
                else
                {
                    return new JsonResult(new { success = false, message = "Failed to update the silver rate." });
                }
            }
            catch (Exception ex)
            {
